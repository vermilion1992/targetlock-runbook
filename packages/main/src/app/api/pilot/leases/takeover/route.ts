...
import { takeoverLeaseInputSchema } from "@/server/pilot/types";
...
export async function POST(request: Request) {
  try {
    ...
    const input = takeoverLeaseInputSchema.parse(await request.json());
    const lease = await getPilotService(environment).takeoverLease(
      context,
      input,
    );
